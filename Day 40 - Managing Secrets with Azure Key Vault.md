...
2. Create a key named devops-key within the Key Vault.
- Key type: `RSA`
- RSA key size: `4096`
...
`az account show --query user.name -o tsv` and grant the required Key Vault permissions.
...
8. In the left navigation, select Objects > Keys > + Generate/Import
...
11.  In the left navigation, Access policies, Click on the `+ Create`  and configure the following
