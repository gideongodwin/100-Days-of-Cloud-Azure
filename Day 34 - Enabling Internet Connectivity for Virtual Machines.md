...
> The SSH key required to access the Azure VM is already created and added to the VM's authorized keys. You can find the SSH key at `/root/.ssh/id_rsa` on the azure-client host.

## Steps:

1. Retrieve the Public IP Address of the VM
   ```
   az vm list-ip-addresses -n datacenter-vm -g $(az group list --query '[].name' --output tsv | grep kml)
   ```
